...
function Catproduct() {
  ...
  useEffect(() => {
    if (name) {
      const filteredProducts = productDetails.filter(product => product.sub_category && product.sub_category.includes(name));
          ...
          {filteredProducts.map((product, index) => (
            <Link to={`/product?id=${product.id}`}><div className="product-Div" key={index} >
              <div className="card h-56 md:h-[150px] w-40 relative overflow-hidden">
                <img src={allApis?.baseurl + product.colorDetails[0].image_url[0].url} loading='lazy' className='h-full w-full object-contain' alt={product.title} />
              </div>
              <div>
